...
exports.obtenerUsuarioXEmail = async (req, res) => {
	try {
		const usuarioData = await usuarioModel.find({ email: req.body.email });
		if (!usuarioData) {
			return res.status(404).send({ msg: 'usuario no encontrado' })
		} else {
			if (usuarioData[0].clave !== req.body.clave) {
				return res.status(404).send({ msg: 'clave incorrecta' })
			} else {
		...
		console.log(error);
		res.status(404).send("Ocurrió algo inesperado");
	}
};
